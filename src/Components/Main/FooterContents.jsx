...
const FooterContents = () => {
	...
	const footerBottomSections =
		locale === "id" ? idData.footerBottomSections : enData.footerBottomSections;
			...
			>
				<footer className="pb-10 border-t border-stone-300 px-5 overflow-hidden">
					...
					<div className="w-full max-w-6xl flex flex-col gap-5 items-center mx-auto text-center -mt-14 mb-10">
						<h1 className="text-2xl font-bold">{lang("brandIntro")}</h1>
						<p
							className="text-stone-600"
							dangerouslySetInnerHTML={{
								__html: lang("brandDescription").replace(
									/Volerhaut/g,
									`<strong>${BRAND}</strong>`
								),
							}}
					...
					<form onSubmit={subscriberSubmit}>
						...
						<div className="relative sm:w-96 max-w-sm w-full sm:max-w-xl mx-auto">
							...
							{loading ? (
								...
								<ArrowRight
									onClick={subscriberSubmit}
									className="absolute top-3 right-3 input-icon p-1"
								/>
							)}
							<label
								htmlFor="search"
								...
								<div key={index} className="flex flex-col gap-2 mx-auto">
									...
									<ul className="text-stone-600 text-sm flex flex-col">
										{section.items.map((item, i) => (
											<li
												key={i}
												className="karma-hover-effect hover:text-stone-800 w-max"
											>
												{item}
											</li>
										))}
